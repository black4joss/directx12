...
void CMeshManager::Create_ConstBuff_Descriptors_Heap_And_View()
{
	...
	cbvHeapDesc.NumDescriptors = m_NumFrameResources;
...
{
	//последний параметр 0 это начальный shader register = b0 для конст.буфера
	CD3DX12_DESCRIPTOR_RANGE cbvTable0;
	cbvTable0.Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 1, 0);

	//последний параметр 0 это начальный shader register = t0 для текстуры
	CD3DX12_DESCRIPTOR_RANGE srvTable1;
	srvTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0);
	
	//два корневых параметра
...
void CMeshManager::DrawRenderItems_Сube(ID3D12GraphicsCommandList* CmdList)
{
	...
	CmdList->SetDescriptorHeaps(_countof(DescriptorHeapsCbv), DescriptorHeapsCbv);
	auto passCbvHandle = CD3DX12_GPU_DESCRIPTOR_HANDLE(m_CbvHeap->GetGPUDescriptorHandleForHeapStart());
	passCbvHandle.Offset(m_CurrFrameResourceIndex, m_CbvSrvUavDescriptorSize);
	//первый параметр 0 значит нулевой индекс в массиве корневых параметров
	//то есть slotRootParameter[0].InitAsDescriptorTable(1, &cbvTable0);
	...
	CmdList->SetDescriptorHeaps(_countof(DescriptorHeapsSrv), DescriptorHeapsSrv);
	//первый параметр 1 значит нулевой индекс в массиве корневых параметров
	//то есть slotRootParameter[1].InitAsDescriptorTable(1, &srvTable1);
