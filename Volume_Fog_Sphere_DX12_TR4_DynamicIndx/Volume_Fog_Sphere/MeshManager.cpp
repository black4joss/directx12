...
void CMeshManager::BuildMaterials()
{
...
{
	m_VsByteCode = d3dUtil::CompileShader(L"Shaders\\tex.hlsl", nullptr, "VS", "vs_5_1");
	m_PsByteCode = d3dUtil::CompileShader(L"Shaders\\tex.hlsl", nullptr, "PS", "ps_5_1");
...
void CMeshManager::Create_ConstBuff_Descriptors_Heap_And_View()
{
	...
	//object constants и pass constants храним в одной куче
	//поэтому objCount + 1
...
{
	//pass CB mat wvp и другое
	CD3DX12_DESCRIPTOR_RANGE cbvTable0;
	cbvTable0.Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 1, 0);

	//object CB mat world для каждого обьекта
	CD3DX12_DESCRIPTOR_RANGE cbvTable1;
	cbvTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 1, 1);

	//shader res для текстур
	CD3DX12_DESCRIPTOR_RANGE srvTable;
	//было 1
	//srvTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0);
	//я сделал 12 текстур
	srvTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 12, 0);

	//shader res для массива материалов


	//const buff для индекса текстуры в массиве материалов


	CD3DX12_ROOT_PARAMETER slotRootParameter[4];

	//mat world и gMaterialIndex для каждого обьекта object CB регистр b0
	slotRootParameter[0].InitAsDescriptorTable(1, &cbvTable0);
	//mat wvp и другое pass CB регистр b1
	slotRootParameter[1].InitAsDescriptorTable(1, &cbvTable1);
	//для текстур 12 штук shader res регист t0 space 0
	slotRootParameter[2].InitAsDescriptorTable(1, &srvTable);
	//материалы materials регистр t0 space 1
...
void CMeshManager::Update_MeshManager()
{
	...
	for (auto& e : m_AllRitems)
	{
		//if (e->NumFramesDirty > 0)
		{
			...
			ObjConstants.MaterialIndex = e->ObjMatIndex;
	...
	DirectX::XMStoreFloat4x4(&ObjConstants.View, DirectX::XMMatrixTranspose(MatView));
...
void CMeshManager::DrawRenderItems_Scene(ID3D12GraphicsCommandList* CmdList, const std::vector<std::unique_ptr<RenderItem>>& Ritems)
{
	...
	for (size_t i = 0; i < Ritems.size(); ++i)
	{
		...
		//конст буфф мат мира для каждого обекта
		...
		//передаем мат мира для текущего объекта
		CmdList->SetGraphicsRootDescriptorTable(0, cbvHandle);

		//----------------------------------------------------------------

		//shader res view массив материалов с ном текст
...
void CMeshManager::Draw_MeshManager()
{
	...
	//передаем mat vwp pass регистр b1
